module GA_Unit
  ...
  type :: TGAHandle
    ...
    ! variables
    ! ------------------------------------------------------
    type(BaseGroup),    pointer :: group              => null()
  ...
  subroutine Init(self, eval)
    ...
    if (btest(mode(4), 1)) then
      select case (mode(1))
      case (0)
        list = Reverse(pack(group%score%key, group%indivs(group%score%key)%rank == 1))
  ...
  subroutine Operate(self)
    class(TGAHandle), intent(inout) :: self
    type(BaseIndiv)                    :: indiv_new
    type(BaseIndiv),     allocatable   :: indivs_new(:)
    type(BaseGroup),     pointer       :: group
    ...
    if (num_elite > 0) then
      ...
      case (0) ! pareto
        if (num_elite >= group%num_elite) then
          ...
          indivs_new(1:num_elite) = pack(group%indivs, group%indivs%rank == 1)
        else
          ...
          indivs_new(2:num_elite) = pack(group%indivs(group%score(pos)%key), &
                                         group%indivs(group%score(pos)%key)%rank == 1)
  ...
  subroutine Operate1(self)
    class(TGAHandle), intent(inout) :: self
    type(BaseIndiv)                    :: indiv_new
    type(BaseIndiv),     allocatable   :: indivs_new(:)
    type(BaseGroup),     pointer       :: group
    type(Group1)                    :: subgroup
   integer,           allocatable   :: list(:)
    integer                         :: mode(4), gener, num_indiv, num_chrom
    integer                         :: lim_create, parents(2), lim, i
    ...
    allocate(indivs_new(100))

    list = pack([(i, i = 1, num_indiv)], group%indivs%rank > 1)

    parents         = Shuffle(list, 2)
    ...
    call subgroup%Eval(mode(3))
    call subgroup%Select
    group%evald = .false.
    group%indivs(parents) = subgroup%indivs
  end subroutine Operate1
  ...
  subroutine Eval(self)
    ...
    if (btest(mode(4), 1)) then
      if (mod(gener, step_output_file) == 0) then
        select case (mode(1))
        case (0)
          list = pack(group%score%key, group%indivs(group%score%key)%rank == 1)
          do i = 1, group%num_elite
  ...
  subroutine Select(self, parents)
    ...
    if (group%num_elite >= 2 .and. mode == 0) then
      parents = SelectPateto(group%num_indiv, group%indivs%rank)
    else
      select case (mode)
      case (0, 1)
  ...
  subroutine Crossover(self, indiv, parents)
    class(TGAHandle), intent(in)    :: self
    type(BaseIndiv),     intent(inout) :: indiv

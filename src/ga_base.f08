module GA_Base
  ...
  type, extends(BaseGroup) :: Group1
    contains
    procedure :: Select => Select1
  end type Group1
  ...
  subroutine Select1(self)
    class(Group1),   intent(inout) :: self
    type(BaseIndiv), allocatable   :: indivs(:)
    type(Hash),      allocatable   :: score_roulette(:)
    real(8),         allocatable   :: rand_array(:)
    integer                        :: num_indiv, n(2), i

    ! if (.not. self%evald) call self%Eval(1)

    allocate(indivs(2))

    num_indiv = self%num_indiv_valid - 1

    rand_array = [(Random(), i = 1, num_indiv)]

    score_roulette       = self%score(2:)
    score_roulette%value = score_roulette%value * rand_array
    score_roulette       = Sort(score_roulette)

    indivs(1) = self%indivs(self%score(1)%key)
    indivs(2) = self%indivs(score_roulette(1)%key)

    call self%Set(indivs)
  ...
  function SelectPateto_t(num_indiv, rank)
    integer              :: SelectPateto_t(2)
    integer, intent(in)  :: num_indiv, rank(:)
    integer, allocatable :: list(:)
    integer              :: n(2), i

    if (count(rank == 2) > 2) then
      list = PACK([(i, i = 1, num_indiv)], rank == 2)
    else
      list = PACK([(i, i = 1, num_indiv)], rank == 2 .or. rank == 3)
    end if
  ...
  function SelectPateto(num_indiv, rank)
    ...
    do
      t = Random(num_indiv)
      if ((i == 1 .or. t /= n(1)) .and. rank(t) > 0 .and. Random() < 1d0 / (1d0 + EXP(rank(t) * 0.3d0))) then
        n(i) = t
        i = i + 1
        if (i > 2) exit
      end if

module GA_Main
  ...
  subroutine GAMain1(handle)
    ...
    integer                                 :: num_island, interval_migration, i


    num_island  =  handle%num_indiv
    ...
    do
      ...
      case (0)
        ...
        allocate(subhandles(num_island))
      ...
      case (2)
        ...
        allocate(indivs(num_island))
        do i = 1, num_island
          indivs(i) = subhandles(i)%indiv_best
        end do
        call handle%group%Set(indivs)
        call handle%Eval
  ...
  subroutine GAMain2(handle)
    type(TGAHandle), intent(inout), pointer :: handle
    type(TGAHandle),                pointer :: subhandles(:)
    type(BaseIndiv),               allocatable :: indivs(:), indivs_temp(:)
    type(BaseGroup),                   pointer :: group, group_sub
    real(8)                                 :: min, mean
    real(8),                    allocatable :: list(:)
    integer                                 :: gener, num_chrom, num_indiv, num_indiv_sub
    integer                                 :: i, j, c, p

    call group_all%Init(0, 0, handle%num_chrom)
    do
      if (ACCESS("stop", "") == 0) exit
      ...
      case (1) ! allocate
        do i = 1, num_indiv
          ...
          do j = 1, num_indiv_sub
            if (j == 1 .and. group_sub%indiv_ref%rank == 1) then
        ...
        j = group_all%num_elite
        ! allocate(indivs_temp(j + sum(subhandles%group%num_elite)))
        ! if (j > 0) &
        !   indivs_temp(1:j) = pack(group_all%indivs, group_all%indivs%rank == 1)
        ! do i = 1, num_indiv
        !   indivs_temp(j + 1:j + subhandles(i)%group%num_elite) = &
        !     pack(subhandles(i)%group%indivs, subhandles(i)%group%indivs%rank == 1)
        !   j = j + subhandles(i)%group%num_elite
        ! end do
        ! stop

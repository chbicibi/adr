...
def predict2d(X, Y, Z, lx, ly, lz):
  ...
  ax = plt.subplots_adjust(left=0.1, bottom=0.1, right=0.95, top=0.95, wspace=0.2, hspace=0.2)
  # ax = Axes3D(fig)
...
def main(argv):
  ...
  x_label = labels[3]
  y_label = labels[4]
  z_label = "$dv$"#["$c_x$", "$c_m$", "$c_z$", "$z$"][int(no) - 1]

  with open(file, "r") as f:
    reader = csv.reader(f)
    next(reader)
    X, Y, Z = (np.array(x, dtype=np.float).reshape((5001, -1)) for x in list(zip(*reader))[0:3])

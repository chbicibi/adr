...
def get_f_call_problem(cdll):
    ...
    def f_(iargs, dargs):
        if not isinstance(iargs, np.ndarray):
            iargs = np.array(iargs)
        if not isinstance(dargs, np.ndarray):
            dargs = np.array(dargs)
        n1 = ctypes.c_int32(len(iargs))
        n2 = ctypes.c_int32(len(dargs))
...
def __test__():
    ...
    # 関数呼び出し
    for i in range(100):
        a = np.array([i+1, i+2], dtype=np.int32)
        b = np.array([0, 1.5, 0.4, 0.3, 0.3, 0.5, 0.5], dtype=np.float64)

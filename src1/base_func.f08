module Base_Func
  ...
  ! private
  ! public :: THash, SetRandomSeed, Random, Sort, Reverse, Shuffle, CheckDup
  ...
  recursive function Sort_h(hash_in) result (hash_out)
    type(THash), allocatable :: hash_out(:)
    type(THash), intent(in)  :: hash_in(:)
    integer                     :: size_hash
    real(8)                     :: pivot
  ...
  function Reverse_h(hash)
    type(THash), allocatable :: Reverse_h(:)
    type(THash), intent(in)  :: hash(:)
    integer                     :: i

    Reverse_h = [(hash(i), i = size(hash), 1, -1)]
  ...
  function Shuffle_a(array)
    integer, allocatable :: Shuffle_a(:)
    integer, intent(in)  :: array(:)
    integer              :: size_array, pos, i
    ...
    do i = 1, size_array
      pos            = Random(i)
      Shuffle_a(i)   = Shuffle_a(pos)
      Shuffle_a(pos) = array(i)
    end do

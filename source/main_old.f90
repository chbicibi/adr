...
contains
  subroutine read_debridata
    ...
    gen(1:db) = int((geny(1:db) - 15) * 365 + gend(1:db)) + mjd
  ...
  subroutine hyouka(order_list, fit, total_RCS, total_flight_time, k)
    ...
    real(8), parameter :: GE = 3.986d14                     ! 地心重力定数
    real(8), parameter :: PI = 3.141592653d0                ! 円周率

    !-----------------------------------出発軌道要素
    integer, parameter :: gen1 = 51544
    real(8), parameter :: incl1 = 30.0d0
    real(8), parameter :: omega1 = 163.967d0
    real(8), parameter :: ecc1 = 0.01704239d0
    real(8), parameter :: argper1 = 297.774d0
    real(8), parameter :: ma1 = 358.189d0
    real(8), parameter :: long_a = 149653496.267d0
    real(8), parameter :: smma = 1.5236781d0
    ...
    do i = 1, n
      ...
      do j = 1, tar
       ...
       & 'debri', select_gen(i, j), smma, select_ecc(i, j), &
       & select_incl(i, j), select_argper(i, j), select_omega(i, j), select_ma(i, j), ref
        ...
        if (j == 1) then
         ...
         & TOF, gen1, long_a, ecc1, omega1, incl1, argper1, ma1, flag
        else
          write(30, '(i4, a, i2, a, i2, i3.2, a, i2.2, a, i2.2, &
         & f12.3, i12, f18.3, f12.8, f12.5, f12.5, f12.5, f12.5, i5, $)') &
         & year, '/', month, '/', day, hour, ':', minute, ':', second, &
         & TOF, select_gen(i, j - 1), long_a, select_ecc(i, j - 1), select_omega(i, j - 1), &
        ...
        !-------------------------------------------------------------------------
        delta_V_d = delta_V_d / 27.6
        delta_V_a = delta_V_a / 27.6

        !-------------------------------合計ΔV算出
        total_delta_V = total_delta_V + delta_V_d + delta_V_a

        !-------------------------------不良個体処理(仮)
        if (delta_V_d + delta_V_a < 0.001) then
...
contains
  subroutine output_result(elite_fit, elite_RCS, c, k, elite_flight_time, elite_group)
    ...
    integer, intent(in) :: elite_group(:, :)                ! 上位5個体
    integer, intent(in) :: c
    integer, intent(in) :: k
    ...
    do i = 1, c
      write(50, '(f12.4, a, f6.4, a, f12.6, 6(a, i3))') &
...
program main
  ...
  do k = 1, T
   ...
   & local_fit, total_RCS, elite_RCS, local_RCS, c, elite_flight_time, total_flight_time)
    ...
    write(*, '(a, i3)') 'elite:', c
    do i = 1, c
      write(*, '(f8.4, a, 5i5)') elite_fit(i), ' km/s', elite_group(i, 1:tar)
    end do
